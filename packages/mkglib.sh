...
/usr/local/bin/meson --cross-file cross_file.txt --prefix ${ANTIX_PKG_BUILD}/glib -D selinux=false -D libmount=false -D internal_pcre=true -D installed_tests=false ./build
cd build
ninja -j 10
ninja install
exit 0
